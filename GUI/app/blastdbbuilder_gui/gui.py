...
class App(tk.Tk):
    ...
    def _summary_indicates_natural_completion(self, cwd: str) -> bool:
        ...
        try:
            ...
            # Concat: you log a line containing "✅" and "Concatenated ... into ..."
            concat_done = (("Concatenated" in text) and (" into " in text) and (("✅" in text) or ("[OK]" in text)))
        ...
        try:
            ...
            if chunk:
                ...
                for line in chunk.splitlines():
                    if ("✅ Downloaded " in line) or ("[OK] Downloaded " in line) or (" Downloaded GCF_" in line) or (" status=finish ok=" in line):
                        saw_noise = True
                        continue

                    # ---- Phase completion detection (concat/build) ----
                    if ("✅" in line) and ("Concatenated" in line) and (" into " in line):

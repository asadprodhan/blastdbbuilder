...
def ensure_container(container_dir, container_name, image):
    ...
    if not os.path.isfile(container_path):
        print(f"[INFO] Downloading container: {container_name}")
        os.makedirs(container_dir, exist_ok=True)
        run_cmd([get_container_engine(), "pull", container_path, image])
...
def download_group(group_name, assembly_url, db_dir, container_dir, summary_log):
    print(f"\n[INFO] Downloading {group_name} genomes into {os.path.join(db_dir, group_name)} ...")
    ...
    datasets_exec = f"{get_container_engine()} exec {datasets_container} datasets"
    ...
    write_summary(summary_log, f"[OK] All genomes processed for {group_name}.")
...
def concat_genomes(db_dir, summary_log):
    ...
    if not fasta_files:
        print("[ERROR] No genome FASTA files found to concatenate")
        return None
    ...
    print(f"[OK] Concatenation done. File moved to {final_fasta}")
    return final_fasta
...
def build_blast_db(fasta_file, summary_log, container_dir, db_dir):
    if not fasta_file or not os.path.isfile(fasta_file):
        print("[ERROR] FASTA file for BLAST DB not found.")
        return
    ...
    cmd = [
        get_container_engine(), "exec", blast_container,
        "makeblastdb",
        "-in", fasta_file,
        "-dbtype", "nucl",
        "-out", db_prefix
    ...
    print(f"[OK] BLAST database built at {db_prefix}")

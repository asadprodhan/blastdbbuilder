...
if wget -q -O "$ASSEMBLY_FILE" https://ftp.ncbi.nlm.nih.gov/genomes/refseq/plant/assembly_summary.txt; then
...
# -----------------------------
OUTPUT_CSV="$GROUP_DIR/plant_reference_genome_${DATE}.csv"
...
for f in temp_part_*.csv; do
    mv "$f" "plant_reference_genome_part${n}_${DATE}.csv"
    ((n++))
done

NUM_PARTS=$(ls plant_reference_genome_part*_${DATE}.csv | wc -l)
...
# -----------------------------
for metadata in plant_reference_genome_part*_${DATE}.csv; do
    echo "======================================" | tee -a "$LOG_FILE"
...
echo "[$(date)] ✅ Plant download completed: $TOTAL_FASTA genomes." | tee -a "$LOG_FILE"

#!/bin/bash --login
# ======================================
# Fungi Reference Genome Downloader (RESUME mode)
...
echo "Downloading Fungi assembly_summary.txt..."
...
for f in "$BASE_DIR"/temp_part_*.csv; do
    mv "$f" "$BASE_DIR/fungal_reference_genome_part${n}_${DATE}.csv"
    ((n++))
done
echo "✅ CSV split into $(ls "$BASE_DIR"/fungal_reference_genome_part*_${DATE}.csv | wc -l) chunks"

# -----------------------------
# 4. Download genomes from CSV
# -----------------------------
for metadata in "$BASE_DIR"/fungal_reference_genome_part*_${DATE}.csv; do
    echo "======================================"
...
echo "✅ All Fungi genomes are now in $BASE_DIR"

...
DB_BASENAME="nt"
SEQKIT_CONTAINER="$HOME/containers/seqkit_2.10.1.sif"
BLAST_CONTAINER="$HOME/containers/ncbi-blast_2.16.0.sif"
...
usage() {
    ...
    echo "  --no-compress                  Do not compress chunks (default: true)"
...
echo "Compress chunks: $COMPRESS_CHUNKS" | tee -a "$LOGFILE"
echo "---------------------------------------------" | tee -a "$LOGFILE"
...
# -----------------------------
# Step 2 — Split FASTA safely with SeqKit if not already split
# -----------------------------
if [ -z "$(ls -A "$CHUNK_TMP" 2>/dev/null)" ]; then
    ...
    echo "⏩ FASTA chunks already exist in $CHUNK_TMP. Skipping splitting." | tee -a "$LOGFILE"
fi
...
for CHUNK in "${CHUNKS[@]}"; do
    ...
    mv "$CHUNK" "$NEW_NAME"
    [ "$COMPRESS_CHUNKS" = true ] && gzip -f "$NEW_NAME" && NEW_NAME="$NEW_NAME.gz"
...
for CHUNK_FILE in "${CHUNKS[@]}"; do
    ...
    BASENAME=${BASENAME%.fna*} # remove .fna or .fna.gz
